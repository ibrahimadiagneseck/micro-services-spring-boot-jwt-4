...
//    // Authentification non personnalisé
//    @PostMapping("/token")
...
//    }
    ...
    @PostMapping("/token")
    public ResponseEntity<Map<String, String>> jwtToken(String grantType, // "password" ou "refreshToken"
                                                        String username,
                                                        String password,
                                                        boolean withRefreshToken,
                                                        String refreshToken) {

        String subject = null; // username
        String scope = null; // roles



        if(grantType.equals("password")) { // username et password
            // ---------------------------------------------------------------------
            ...
            scope = authentication.getAuthorities()
                    ...
                    .collect(Collectors.joining(" "));
            // ---------------------------------------------------------------------
        } else if(grantType.equals("refreshToken")) { // avec un refreshToken
            // ---------------------------------------------------------------------
            ...
            try {
                decodeJWT = jwtDecoder.decode(refreshToken); // decoder le refreshToken
            } catch (JwtException e) {
                return new ResponseEntity<>(Map.of("errorMessage", e.getMessage()), HttpStatus.UNAUTHORIZED);
            }

            subject = decodeJWT.getSubject(); // pas de username
        ...
        if(withRefreshToken) { // pour les applications mobile
            // ---------------------------------------------------------------------
